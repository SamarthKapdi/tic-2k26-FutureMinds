...
const list = async (req, res) => {
  try {
    ...
    const result = await missingService.listMissing(parseInt(page), parseInt(limit), city, status || 'missing');

    return res.status(200).json({ success: true, data: result });
  } catch (error) {
    console.error('List missing error:', error);
    return res.status(500).json({ success: false, message: 'Failed to load missing persons' });
  }
};
...
const sighting = async (req, res) => {
  try {
    const { missing_person_id, latitude, longitude, address, description, image_url, sighting_date } = req.body;
    ...
    const result = await missingService.reportSighting(req.user.id, {
      missing_person_id, latitude, longitude, address, description, image_url, sighting_date,
    });
