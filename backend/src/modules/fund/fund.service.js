...
const getCategories = async () => {
  const result = await query(
    'SELECT * FROM fund_categories WHERE is_active = true ORDER BY name'
  )
  return result.rows
}
...
const createCampaign = async (userId, data) => {
  ...
  if (duplicate.rows.length > 0) {
    throw new Error(
      'A similar campaign already exists. Please check your active campaigns.'
    )
  }

  const result = await query(
    `INSERT INTO fund_campaigns (creator_id, category_id, title, description, target_amount, image_url, documents_url, urgency, end_date)
    ...
    [
      ...
      image_url || null,
      Array.isArray(documents_url) && documents_url.length > 0
        ? documents_url
        : null,
      urgency || 'normal',
      end_date || null,
...
const createDonationOrder = async (campaignId, donorData) => {
  ...
  const campaign = await query(
    'SELECT * FROM fund_campaigns WHERE id = $1 AND status = $2',
    [campaignId, 'active']
  )
  ...
  if (!cashfreeAppId || cashfreeAppId === 'your_cashfree_app_id') {
    ...
    console.log(
      '⚠️ [DEV MODE] Cashfree credentials not configured. Returning mock order.'
    )
    return {
      order_id: orderId,
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        cashfreeOrder.message || 'Failed to create Cashfree order'
      )
    }
    ...
    await query(
      "UPDATE fund_transactions SET payment_status = 'failed' WHERE id = $1",
      [transaction.rows[0].id]
    )
    throw error
  }
  ...
  try {
    ...
    const txResult = await client.query(
      ...
      [
        ...
        data.payment?.payment_group || null,
        orderId,
      ]
    )
    ...
    if (dbStatus === 'success') {
      ...
      const campaign = await client.query(
        'SELECT * FROM fund_campaigns WHERE id = $1',
        [tx.campaign_id]
      )
      if (
        campaign.rows[0] &&
        parseFloat(campaign.rows[0].raised_amount) >=
          parseFloat(campaign.rows[0].target_amount)
      ) {
        await client.query(
          "UPDATE fund_campaigns SET status = 'completed', updated_at = NOW() WHERE id = $1",
          [tx.campaign_id]
        )
      }
      ...
      if (tx.donor_id) {
        await client.query(
          'UPDATE users SET trust_score = LEAST(trust_score + 2, 100) WHERE id = $1',
          [tx.donor_id]
        )
      }
    }
