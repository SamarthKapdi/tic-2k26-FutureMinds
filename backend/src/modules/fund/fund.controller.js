...
const { uploadFundCampaignAsset } = require('../../services/r2.service')
...
const createCampaign = async (req, res) => {
  try {
    ...
    if (req.files?.campaign_image?.[0]) {
      resolvedImageUrl = await uploadFundCampaignAsset(
        req.files.campaign_image[0],
        'fund/images'
      )
    }

    if (req.files?.campaign_proof?.[0]) {
      const proofUrl = await uploadFundCampaignAsset(
        req.files.campaign_proof[0],
        'fund/proofs'
      )
...
const webhookCashfree = async (req, res) => {
  try {
    const signature =
      req.headers['x-cashfree-signature'] || req.headers['x-webhook-signature']
    const result = await fundService.handleWebhook(req.body, signature)

    const io = req.app.get('io')
    if (io && result?.status === 'success') {
      io.emit('dashboard:refresh')
      io.emit('leaderboard:refresh')
    }
