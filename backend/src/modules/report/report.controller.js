...
const create = async (req, res) => {
  try {
    ...
    return res
      .status(error.message.includes('already reported') ? 409 : 500)
      .json({
        success: false,
...
/**
 * GET /api/report/active-count
 */
const getActiveCount = async (req, res) => {
  try {
    const active_reports = await reportService.getActiveReportsCount()
    ...
    console.error('Active reports count error:', error)
    return res.status(500).json({
      success: false,
      message: 'Failed to load active reports count',
    })
  }
}

module.exports = { create, list, getActiveCount }
