    ...
    <div className="min-h-[80vh] flex items-center justify-center">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-secondary-dark to-secondary flex items-center justify-center mx-auto mb-4 shadow-lg shadow-secondary/20">
            <Shield className="h-8 w-8 text-white" />
          </div>
          <h1 className="font-heading text-2xl font-bold text-text">Admin Panel</h1>
          <p className="text-text-secondary mt-1">Login with your admin credentials</p>
        </div>
        <Card hover={false}>
          {error && <div className="mb-4 p-3 rounded-xl bg-red-50 border border-red-200 text-sm text-red-700">{error}</div>}
          <form onSubmit={handleLogin} className="space-y-5">
            <Input id="admin_email" label="Email" type="email" placeholder="admin@sahyog.org" value={email} onChange={(e) => setEmail(e.target.value)} required />
        ...
        <p className="text-center text-xs text-text-muted mt-4">Default: admin@sahyog.org / admin123</p>
      </motion.div>
    </div>
  );
}
