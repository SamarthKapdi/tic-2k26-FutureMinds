...
export default function AdminDashboard() {
  ...
  const handleApprove = async (id) => {
    setActionLoading(id);
    try {
      await adminAPI.approveVerification({ verificationId: id, notes: 'Approved by admin' });
      loadData();
    } catch (err) {
  ...
  const handleReportAction = async (id, action) => {
    const notes = action === 'dismissed' ? '' : (prompt('Notes (optional):') || '');
    setActionLoading(id);
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-secondary-dark to-secondary flex items-center justify-center">
            <Shield className="h-5 w-5 text-white" />
          </div>
          <div>
            <h1 className="font-heading text-2xl font-bold text-text">Admin Panel</h1>
            <p className="text-sm text-text-secondary">Welcome, {admin.name || 'Admin'}</p>
          </div>
        </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <StatCard icon={FileCheck} label="Pending Verifications" value={stats?.pending_verifications || 0} color="accent" />
            <StatCard icon={AlertTriangle} label="Pending Reports" value={stats?.pending_reports || 0} color="danger" />
      ...
      {activeTab === 'verifications' && (
        <div className="space-y-4">
          {verifications.length === 0 ? (
            <Card hover={false}><p className="text-text-muted text-center py-8">No verification requests</p></Card>
          ) : (
            verifications.map((v) => (
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge variant={v.status === 'pending' ? 'urgent' : v.status === 'approved' ? 'success' : 'critical'}>
                        {v.status}
                      </Badge>
                    </div>
                    <p className="text-sm text-text-secondary">Document: {v.document_type} · {v.user_phone}</p>
                    <p className="text-xs text-text-muted">{timeAgo(v.created_at)}</p>
                  </div>
                  {v.status === 'pending' && (
                    <div className="flex gap-2">
                      <Button size="sm" variant="success" loading={actionLoading === v.id} onClick={() => handleApprove(v.id)}>
                        <CheckCircle className="h-3.5 w-3.5" /> Approve
                      </Button>
                      <Button size="sm" variant="danger" loading={actionLoading === v.id} onClick={() => handleReject(v.id)}>
      ...
      {activeTab === 'reports' && (
        <div className="space-y-4">
          {reports.length === 0 ? (
            <Card hover={false}><p className="text-text-muted text-center py-8">No reports</p></Card>
          ) : (
            reports.map((r) => (
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge variant={r.status === 'pending' ? 'urgent' : 'success'}>{r.status}</Badge>
                    </div>
                    <p className="text-sm font-medium text-text">{r.reason}</p>
                    <p className="text-xs text-text-secondary mt-1">by {r.reporter_name} · {timeAgo(r.created_at)}</p>
                  </div>
                  {r.status === 'pending' && (
                    <div className="flex gap-2">
                      <Button size="sm" variant="success" loading={actionLoading === r.id} onClick={() => handleReportAction(r.id, 'resolved')}>
                        Resolve
                      </Button>
                      <Button size="sm" variant="ghost" loading={actionLoading === r.id} onClick={() => handleReportAction(r.id, 'dismissed')}>
                        Dismiss
                      </Button>
                    </div>
                  )}
